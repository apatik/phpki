...
<table>
	...
	<tr>
		<td colspan=8><center>
				<form action="{$PHP_SELF}?{$qstr_sort}" method="get" name="filter">
					Search: <input type="text" name="search" value="{$search}" style="font-size: 11px;" maxlength=60 size=30>
	...
	{foreach from=$db item=rec}
		<tr style="font-size: 11px;">
			...
			<td style="white-space: nowrap"><a href="mailto:{$rec.common_name} <{$rec.email}>">{$rec.email}</a></td>
			<td>{$rec.organization}</td>
			<td>{$rec.unit}</td>
			<td>{$rec.locality}</td>
			<td>
				<a href="{$PHP_SELF}?stage=display&serial={$rec.serial}" target="_certdisp"><img src="../images/display.png" alt="Display" title="Display complete certificate details."></a>
				...
				{/if}
				<a href="{$PHP_SELF}?stage=renew-form&serial={$rec.serial}&{$qstr_sort}&{$qstr_filter}"><img src="../images/renew.png" alt="Renew" title="Renew the certificate by revoking it, if necessary, and creating a replacement with a new expiration date."></a>
			</td>
		</tr>
	{/foreach}
</table>