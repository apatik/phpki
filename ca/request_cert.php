    ...
    case 'validate':
        ...
        foreach ($dns_n as $value){
            ...
            if ( $value && ! is_fqdn(trim($value)) )
                $warn[] = 'DNS Name ('. htvar($value) . ') may not be valid.';
        }
    ...
    case 'confirm':
        ...
        switch  ($cert_type) {
            ...
            case 'server': $cert_usage = 'SSL Server';
                break;
            case 'client_server': $cert_usage = 'SSL Server + SSL Client';
                break;
    ...
    case 'final':
        if ($submit == "Yes!  Create and Download") {
            ...
            switch($cert_type)
            {
                case 'client_server':
                case 'server':
                    $fileName = (empty($email)) ? $common_name . '.pem' : $common_name . ' (' . $email . ').pem';
                    upload(array("$config[private_dir]/$serial-key.pem","$config[new_certs_dir]/$serial.pem",$config['cacert_pem']), $fileName, 'application/pkix-cert');
                    break;
                case 'email':
                case 'email_signing':
                case 'time_stamping':
